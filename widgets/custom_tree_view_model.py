        ...
        if 'major_aircraft_weight' in data:
            for weight_key, weight_value in data['major_aircraft_weight'].items():
                if weight_key == 'operation_item' and 'operation_items' in data and data['operation_items']:
                    ...
                    for item_weight_value in data['operation_items']:
                        # 这样做是为了避免需改原始数据
                        weight_value_list = [i for i in item_weight_value]
                        weight_value_list.append(None)
                        operation_item.append_child(TreeItemBase(weight_value_list, operation_item))
                if weight_key == 'stowage_item' and 'stowage_items' in data and data['stowage_items']:
                    ...
                    for item_weight_value in data['stowage_items']:
                        weight_value_list = [i for i in item_weight_value]
                        weight_value_list.append(None)
                        stowage_item.append_child(TreeItemBase(weight_value_list, stowage_item))
...
# 使用项目树模型
class FuelConsumptionTreeModel(BaseTreeModel):

    def __init__(self, data_list: list, parent=None):
        super().__init__(data_list, parent, header=['左机翼油箱', '中央翼油箱', '右机翼油箱'])
